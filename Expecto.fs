...
module Log =
  ...
  let message (t:LogType) (msg:string)= printf "%s\n" msg
  let debug msg = message TDebug msg
  let info msg = message TInfo msg
  let warning msg = message TWarning msg
  let error msg = message TError msg
...
module Impl =
  ...
  type TestPrinters =
    ...
    static member Default = {
        ...
        beforeEach = fun n ->
          let msg:string = sprintf "%s starting..." n 
          Log.debug msg
        passed = fun n d ->
          Log.debug (sprintf "%s passed in %O." n d)
        ignored = fun n m ->
          Log.warning (sprintf "%s was ignored. %s" n m)
        failed = fun n m d ->
          Log.error (sprintf "%s failed in %O. %s" n d m)
        exn = fun n e d ->
          Log.error (sprintf "%s errored in %O exception %O" n d e)
        summary = fun summary ->
          Log.info (sprintf "%i tests run in %A \n    %i passed, %i ignored, %i failed, %i errored." summary.total summary.duration summary.passed summary.ignored summary.failed summary.errored) }
